...
fn organize_dir(dir_path: PathBuf){
    if !dir_path.exists(){
        ...
        let dir_files = match dir_path.read_dir(){
            ...
            Err(err) => {
                println!("Error opening the dir \"{}\": \"{}\"", dir_path.display(), err);
                return;
            }
        };
        ...
        for file in dir_files{
            if let Ok(file) = file{
                ...
                let file_extension = match file.path().extension(){
                    None => {
                        println!("Can't get extenshion of the file \"{}\"", file.path().display());
                        continue;
                    }
...
fn create_dir_if_not_exists(dir_path: &PathBuf){
    if !dir_path.exists(){
        if let Err(err) = fs::create_dir(dir_path){
            println!("Error crating dir \"{}\": \"{}\"", dir_path.display(), err);
        }
    }
}
